...
files. It walks a directory tree (default: current working directory), finds
all `.idea/workspace.xml`, and rewrites the `<option name="content" ...>` value
to the prompt you provide.
...
Legacy wrapper
- You can also run `python set-prompt.py --prompt "…" --root ./projets` if you
  prefer calling the top-level script directly.
...
- For each `<AIAssistantStoredInstruction>` block where
  ...
  is present, it sets the sibling `<option name="content" value="…" />` to the
  provided prompt.
